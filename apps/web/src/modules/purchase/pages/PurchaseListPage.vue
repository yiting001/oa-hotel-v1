...
const purchaseDocuments = computed(() =>
  workflow.documents.filter((document) => document.documentType === 'PURCHASE_APPROVAL'),
);

const filteredDocuments = computed(() => {
  const normalizedKeyword = keyword.value.trim().toLocaleLowerCase();
  return purchaseDocuments.value.filter((document) => {
...
const metricItems = computed(() => [
  { key: 'all', label: '全部单据', value: purchaseDocuments.value.length },
  {
    key: 'attention',
    label: '待完善',
    value: purchaseDocuments.value.filter((document) =>
  ...
  },
  {
    key: 'reviewing',
    label: '审批中',
    value: purchaseDocuments.value.filter((document) => document.status === 'IN_REVIEW').length,
  },
  {
    key: 'approved',
    label: '已通过',
    value: purchaseDocuments.value.filter((document) => document.status === 'APPROVED').length,
  },
]);
...
async function refresh(): Promise<void> {
  try {
    ...
    message.error(error instanceof Error ? error.message : '采购单据加载失败');
  }
}
...
<template>
  <div class="purchase-list-page">
      ...
      <template #actions>
        <a-space wrap>
          <a-button
            v-if="canCreate"
            type="primary"
            @click="router.push({ name: PURCHASE_ROUTE_NAMES.create })"
          >
    ...
    <WorkspaceMetricStrip :items="metricItems" label="采购单据统计" />

    <WorkspaceFilterBar :result-label="`共 ${filteredDocuments.length} 条`" label="采购单据筛选">
      <template #search>
