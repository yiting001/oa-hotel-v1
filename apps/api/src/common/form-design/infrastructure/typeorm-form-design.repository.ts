...
import type {
  CreateFormDefinitionRecord,
  FormDesignRepository,
  UpdateFormVersionRecord,
...
@Injectable()
export class TypeOrmFormDesignRepository implements FormDesignRepository {
  constructor(
    ...
    private readonly versions: Repository<FormVersionEntity>,
    @Inject(DataSource)
    private readonly dataSource: DataSource,
  ) {}

  async listDefinitions(): Promise<FormDefinitionDetail[]> {
    ...
    const versionsByDefinition = new Map<string, FormVersionEntity[]>();
  ...
  async findDefinitionDetail(id: string): Promise<FormDefinitionDetail | null> {
    const definition = await this.definitions.findOneBy({ id });
    return definition ? this.withVersions(definition) : null;
  }

  async findPublishedByDocumentType(documentType: string): Promise<PublishedFormDesign | null> {
  ...
  async createDefinition(record: CreateFormDefinitionRecord): Promise<void> {
    await this.dataSource.transaction(async (manager) => {
      await manager.getRepository(FormDefinitionEntity).save(record.definition);
      await manager.getRepository(FormVersionEntity).save({
        ...record.version,
        publishedAt: null,
      });
    });
  ...
  async copyVersion(
    ...
    return this.dataSource.transaction(async (manager) => {
      const repository = manager.getRepository(FormVersionEntity);
      ...
      return repository.save({
        ...
        printSchemaJson: source.printSchemaJson,
        changeNote,
        createdBy: actorId,
        updatedBy: actorId,
        publishedAt: null,
  ...
  async updateDraftVersion(
    ...
    const result = await this.versions.update(
      ...
      {
        ...update,
        schemaJson: update.schemaJson as never,
        printSchemaJson: update.printSchemaJson as never,
      },
    );
    return result.affected === 1 ? this.versions.findOneBy({ id: versionId }) : null;
  }

  async publishDraftVersion(versionId: string, actorId: string): Promise<FormVersion | null> {
    return this.dataSource.transaction(async (manager) => {
      const repository = manager.getRepository(FormVersionEntity);
  ...
  private async withVersions(definition: FormDefinitionEntity): Promise<FormDefinitionDetail> {
    const versions = await this.versions.find({
